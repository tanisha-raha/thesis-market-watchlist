...
export default async function DigestPage() {
  ...
  return <AppShell email={user.email} displayName={user.displayName} active="digest" rows={rows} demo={process.env.THESIS_DATA_MODE === "demo"}>
    ...
    <div className="page-heading"><div><p className="eyebrow">YOUR SINCE-LAST-VISIT DIGEST</p><h1>While you were away</h1><p>{d.awayFrom ? `Since ${formatIST(d.awayFrom)} IST` : "Since you started watching"}{" · "}{d.marketClosedThroughout ? "No trading sessions in this window" : `${d.sessionsInWindow} trading ${d.sessionsInWindow === 1 ? "session" : "sessions"}`}</p></div><Link className="button-secondary" href="/">Home <Icon name="arrow" size={14} /></Link></div>
